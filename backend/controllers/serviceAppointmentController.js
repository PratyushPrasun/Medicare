...
export const confirmServicePayment = async (req,res) =>{
    try {
      ...
      let session ;
      try {session = await stripe.checkout.retrieve(session_id);}
      catch(err){
...
export const cancelServiceAppointment = async (req,res) => {
  try {
    ...
    console.error("cancelServicePayment unexpected error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export const getServiceAppointmentStats = async (req,res) => {
  try {
    ...
    console.error("Get service appintment stats unexpected error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export const getServiceAppointmentByPatient = async (req,res) => {
  try {
    ...
    console.error("Get service appintment By Patient unexpected error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
