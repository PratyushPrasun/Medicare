...
export async function getAppointmentsByPatient(req, res) {
    try {
        ...
        console.log('resolveCreatedBy (query or req.auth.userid): ', resolvedCreatedBy);
...
export const createAppointment = async (req,res) =>{
    ...
    if(!doctorId || !patientName || !mobile || !date || !time){ return res.status(400).json({
        success:false,
    ...
    try {
      const created = await Appointment.create({
        ...
        payment: { ...base.payment, providerId: session.payment_intent || session.payment_intent || null },
        status: "Pending",
      });
...
export const updateAppointment = async(req, res) =>{
    try {
        ...
        if(!appt) return req.status(404).json({
            success:false,
...
export const getAppointmentsByDoctor = async (req,res) => {
  ...
  return res.json({
    success: true,
    appointments, // ✅ actual data
    meta: {
      page,
      limit,
      total,
      count: appointments.length // ✅ correct
    }
  });

} catch (err) {
  console.error("get aap by doc error:", err);
  return res.status(500).json({
    success: false,
    message: "Server error"
  });
